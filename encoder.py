...
def create_generator_encoder(generator_inputs, a):
    ...
    for out_channels in layer_specs:
        with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
            ...
            output = batchnorm(convolved, is_training=True)
            # output = batchnorm(convolved, is_training = a.mode == "train)
            layers.append(output)
...
def create_zs_vae_shared_encoder(input1, input2, is_training, a):
    ...
    return q_z_shared_mean, q_z_shared_logvar
    # return tf.nn.tanh(q_z_shared_mean), tf.nn.tanh(q_z_shared_logvar)


'''
############################## Custom (3-layered Ex, 3-layered Shared w/ param sharing, 256) ##############################
def create_zs_vae_individual_exclusive_encoder(input, is_training, a):
    layers = []

    # encoder_1: [batch, 4096] => [batch, neef * 8]
    with tf.variable_scope("vae_individual_exclusive_encoder_1"):
       output = gen_fc(input, out_channels=a.neef * 8)
       layers.append(output)

    layer_specs = [
        a.neef * 4, # encoder_2: [batch, neef * 8] => [batch, neef * 4]
        a.neef * 2, # encoder_3: [batch, neef * 4] => [batch, neef * 2]
    ]
...
def create_zs_vae_individual_shared_encoder(input, is_training, a):
    ...
    # encoder_1: [batch, 4096] => [batch, nsef * 8]
    with tf.variable_scope("vae_individual_shared_encoder_1"):
       output = gen_fc(input, out_channels=a.nsef * 8)
       layers.append(output)

    layer_specs = [
        a.nsef * 4, # encoder_2: [batch, nsef * 8] => [batch, nsef * 4]
        a.nsef * 2, # encoder_3: [batch, nsef * 2] => [batch, nsef * 2]
    ]
...
def create_zs_vae_shared_encoder(input1, input2, is_training, a):
    ...
    layer_specs = [
        # a.nsef * 4, # encoder_2: [batch, nsef * 4] => [batch, nsef * 4]
        a.nsef * 2, # encoder_3: [batch, nsef * 4] => [batch, nsef * 2]
    ]
...
'''
############################## Custom (2-layered w/o Param Sharing) ##############################
def create_zs_vae_individual_exclusive_encoder(input, is_training, a):
    layers = []

    # encoder_1: [batch, 4096] => [batch, neef * 4]
    with tf.variable_scope("vae_individual_exclusive_encoder_1"):
       output = gen_fc(input, out_channels=a.neef * 4)
       layers.append(output)

    layer_specs = [
        # a.neef * 4, # encoder_2: [batch, neef * 4] => [batch, neef * 4]
        a.neef * 2, # encoder_3: [batch, neef * 4] => [batch, neef * 2]
    ]
...
def create_zs_vae_individual_shared_encoder(input, is_training, a):
    ...
    # encoder_1: [batch, 4096] => [batch, nsef * 4]
    with tf.variable_scope("vae_individual_shared_encoder_1"):
       output = gen_fc(input, out_channels=a.nsef * 4)
       layers.append(output)
...
def create_zs_vae_shared_encoder(input1, input2, is_training, a):
    ...
    # encoder_1: [batch, 4096 + 4096] => [batch, nsef * 4]
    with tf.variable_scope("vae_shared_encoder_1"):
       output = gen_fc(tf.concat([input1, input2], axis=-1), out_channels=a.nsef*4)
       layers.append(output)
